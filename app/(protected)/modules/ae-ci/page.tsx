...
export default function AECIModule() {
  ...
  const next = () => { setIndex((i) => (i + 1) % drugs.length); setShowAE(false); setShowCI(false); };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="bg-white shadow rounded p-6">
        <div className="mb-2">
          <div className="text-2xl font-bold">{language === "en" ? current.name.en : current.name.cs}</div>
          <div className="text-gray-600">{language === "en" ? current.class.en : current.class.cs}</div>
        </div>
        <div className="flex gap-2 mb-4">
          <button onClick={() => setShowAE((v) => !v)} className="px-4 py-2 bg-blue-600 text-white rounded">{showAE ? "Hide" : "Show"} Adverse Effects</button>
          <button onClick={() => setShowCI((v) => !v)} className="px-4 py-2 bg-purple-600 text-white rounded">{showCI ? "Hide" : "Show"} Contraindications</button>
