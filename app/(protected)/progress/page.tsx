        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td className="p-6 text-center text-gray-600" colSpan={7}>No progress yet. Try the modules to get started.</td>
                </tr>
              ) : (
                items.map((it) => {
                  ...
                  return (
                    <tr key={`${it.module}-${it.courseBlock}-${it.tag}`} className="border-t">
                      ...
                      <td className="p-3">{new Date(it.lastAttemptAt ?? Date.now()).toLocaleString()}</td>
                    </tr>
                  );
                })
              )}
